...
/** Same browser storage as Admin + LoanOverview (ADMIN_KEY / LOANOVERVIEW_ADMIN_KEY on server). */
const KEY_STORAGE = 'kopanow_dashboard_auth';
const LEGACY_ACCOUNTING_KEY = 'kopanow_accounting_key';
...
function getKey() {
  try {
    ...
    if (!k) {
      const legacy = sessionStorage.getItem(LEGACY_ACCOUNTING_KEY) || '';
      if (legacy) {
        k = legacy;
        localStorage.setItem(KEY_STORAGE, legacy);
        sessionStorage.removeItem(LEGACY_ACCOUNTING_KEY);
      }
    }
    return k;
  } catch (_) {
