...
const { enforceMinSupportedAppVersion } = require('../helpers/versionGate');
const {
...
router.post('/enrollment-check', async (req, res) => {
  try {
    ...
    const { device_id, imei, imeis, hardware_serial, borrower_id, loan_id, phone, mpesa_phone } =
      req.body || {};
    ...
    const result = await assertIdentifiersFreeForEnrollment(
      device_id,
      imei,
      borrower_id,
      loan_id,
...
// ─────────────────────────────────────────────────────────────────────────────
// POST /api/device/serial
// Onboarding step 8: attach hardware serial after MDM register.
// ─────────────────────────────────────────────────────────────────────────────
router.post('/serial', async (req, res) => {
  try {
    ...
    const { borrower_id, loan_id, hardware_serial, device_id, imei, imeis } = req.body || {};
    ...
    if (!serial) {
      return res.status(400).json({
        success: false,
        error: 'hardware_serial is required and must be readable from the device',
      });
    }

    const enrollment = await assertIdentifiersFreeForEnrollment(
      device_id,
      imei,
      borrower_id,
      loan_id,
    ...
    const { data: existingRow, error: fetchErr } = await supabase
      .from('devices')
      .select('id, device_info, imei, imeis')
      .eq('borrower_id', borrower_id)
      .eq('loan_id', loan_id)
      .maybeSingle();
    if (fetchErr) throw fetchErr;

    const mergedImeis = mergeStoredImeis(normalizeImeiList(imeis, imei), existingRow);
    const primaryImei =
      mergedImeis[0] ||
      normalizeImei(imei) ||
      (existingRow?.imei != null ? normalizeImei(existingRow.imei) : '') ||
      null;

    if (!existingRow) {
      const device_info = {
        hardware_serial: serial,
        hardware_serial_captured_at: now,
        ...(mergedImeis.length ? { imeis: mergedImeis } : {}),
      };
      ...
      console.log(`[device/serial] borrower=${borrower_id} loan=${loan_id} created`);
      return res.json({ success: true, hardware_serial: serial });
    }
    ...
    console.log(`[device/serial] borrower=${borrower_id} loan=${loan_id} updated`);
    return res.json({ success: true, hardware_serial: serial });
  } catch (err) {
...
router.post('/register', async (req, res) => {
  try {
    ...
    const {
      borrower_id, loan_id, fcm_token, device_model, device_id, imei, imeis, hardware_serial, mpesa_phone,
    ...
    const fcmTrimmed = fcm_token != null ? String(fcm_token).trim() : '';
    if (!fcmTrimmed) {
      return res.status(400).json({
        success: false,
        error: 'fcm_token is required — disbursement queue and remote lock commands need push delivery.',
      });
    }

    const mpesaPhoneNorm = mpesa_phone ? normalizePhone(mpesa_phone) : null;
    if (mpesaPhoneNorm) {
    ...
    const imeisNorm = normalizeImeiList(imeis, imei);
    const imeiNorm = imeisNorm[0] || normalizeImei(imei);
    const serialNorm = normalizeHardwareSerial(hardware_serial);
    const enrollment = await assertIdentifiersFreeForEnrollment(
      device_id,
      imei,
      borrower_id,
    ...
    const { data: existingRow } = await supabase
      .from('devices')
      .select('device_info, imei, imeis, hardware_serial')
      .eq('borrower_id', borrower_id)
      .eq('loan_id', loan_id)
      .maybeSingle();
    ...
    const device_info = {
      ...
      ...(serialNorm ? { hardware_serial: serialNorm } : {}),
      ...(imeisNorm.length ? { imeis: imeisNorm } : {}),
    };

    const imeisToStore = mergeStoredImeis(imeisNorm, existingRow);
    const imeiToStore =
      imeisToStore[0] ||
      imeiNorm ||
      (existingRow?.imei != null ? String(existingRow.imei).trim() : null) ||
      null;
    const serialToStore =
      serialNorm ||
      (existingRow?.hardware_serial != null ? String(existingRow.hardware_serial).trim() : null) ||
      null;
    ...
    const { data: device, error: devErr } = await supabase
      ...
      .upsert({
        ...
        imei:         imeiToStore,
        imeis:        imeisToStore,
        hardware_serial: serialToStore,
        fcm_token:    fcmTrimmed,
...
router.post('/heartbeat', async (req, res) => {
  try {
    const {
      borrower_id, loan_id, device_id, imei, imeis, dpc_active, is_safe_mode, battery_pct, timestamp,
    ...
    const imeisHb = normalizeImeiList(imeis, imei);
    const mergedImeisHb = mergeStoredImeis(imeisHb, device);
    if (mergedImeisHb.length) {
      if (!device.imei && mergedImeisHb[0]) updates.imei = mergedImeisHb[0];
      const existingImeis = Array.isArray(device.imeis) ? device.imeis : [];
      if (!existingImeis.length) updates.imeis = mergedImeisHb;
    }
    if (mergedImeisHb.length) {
      const prevInfo =
        device.device_info && typeof device.device_info === 'object' ? device.device_info : {};
      updates.device_info = { ...prevInfo, imeis: mergedImeisHb };
    }
    if (
    ...
    if (updates.protection_first_completed_at) {
      ...
      // Cash disbursement queue: only enqueue when we have push reachability.
      // Without fcm_token the row must not appear — cashier flows assume FCM-backed device contact.
      let skipDisbursementQueue = false;
      if (phoneRaw && (await isPhoneBlockedForDisbursement(phoneRaw))) {
        skipDisbursementQueue = true;
        console.warn(
          `[heartbeat:cash_disbursement_queue] skip: phone on disbursement blocklist (borrower=${borrower_id} loan=${loan_id})`
        );
      }
      if (
        !skipDisbursementQueue &&
        (await isAnyDeviceIdentifierBlocked([
          { device_id, imei, imeis: imeisHb },
          { device_id: device.device_id, imei, imeis: imeisHb },
          { device_id, imei: device.imei, imeis: device.imeis },
          { device_id: device.device_id, imei: device.imei, imeis: device.imeis },
        ]))
      ) {
        skipDisbursementQueue = true;
        console.warn(
          `[heartbeat:cash_disbursement_queue] skip: device or IMEI on disbursement blocklist (borrower=${borrower_id} loan=${loan_id})`
        );
      }
      if (eligible && device.fcm_token && !skipDisbursementQueue) {
        const ts = receivedAt;
      ...
      } else if (eligible && !skipDisbursementQueue) {
        console.warn(
          `[heartbeat:cash_disbursement_queue] skip: missing fcm_token (borrower=${borrower_id} loan=${loan_id})`
        );
      }
    }
...
router.post('/status', async (req, res) => {
  try {
    ...
    const VALID = ['registered', 'active', 'locked', 'admin_removed', 'suspended', 'withdrawn'];
