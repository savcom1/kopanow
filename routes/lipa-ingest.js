...
} = require('../helpers/mixxLipaSmsMarkers');
 ...
 *   raw_sms           (optional) — full SMS text for audit; when present with sms_concatenated_body,
 *                     combined text must contain "Umepokea" and "Lipa Kwa Simu" (Mixx Lipa na simu), case-insensitive.
 *                     Set LIPA_INGEST_REQUIRE_MIXX_MARKERS=1 to require SMS fields on every POST.
...
router.post('/transactions', async (req, res) => {
  try {
    ...
    const smsCombined = combineSmsBodyForMarkers(raw_sms, sms_concatenated_body);
    const requireMixxMarkersAlways =
      String(process.env.LIPA_INGEST_REQUIRE_MIXX_MARKERS || '').trim() === '1';
    if (requireMixxMarkersAlways && !smsCombined) {
      return res.status(400).json({
        success: false,
        error:
          'LIPA_INGEST_REQUIRE_MIXX_MARKERS=1: raw_sms or sms_concatenated_body is required for Mixx Lipa SMS validation',
      });
    }
    if (smsCombined && !hasMixxLipaMarkers(smsCombined)) {
      return res.status(400).json({
        success: false,
        error:
          'SMS must contain Umepokea and Lipa Kwa Simu (Mixx Lipa na simu); case-insensitive',
      });
    }
