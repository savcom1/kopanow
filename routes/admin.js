  ...
  try {
    ...
    if (pendingLoanIds.length) {
      const { data: readyRows, error: rErr } = await supabase
        .from('devices')
        .select('loan_id, fcm_token')
        .in('loan_id', pendingLoanIds)
        .not('protection_first_completed_at', 'is', null)
        .limit(50000);
      if (rErr) throw rErr;
      // Align with heartbeat: pending cashout without FCM is not cashier-ready (not in queue until token exists).
      readyLoanIds = new Set(
        (readyRows || [])
          .filter((r) => r.loan_id && r.fcm_token && String(r.fcm_token).trim())
          .map((r) => r.loan_id)
      );
    }
